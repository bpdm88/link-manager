...
router.post('/', async (req, res) => {
  try {
    ...
    if (password !== passwordVerify) {
      return res
        .status(400)
        .json({ errorMessage: 'Please enter the same password twice for verification' })
    }
...
router.post('/login', async (req, res) => {
  try {
    ...
    if (!correctPassword) return res.status(401).json({ errorMessage: 'wrong email or password' })
